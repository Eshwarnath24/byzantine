...
const (
	...
	maxSendFailures  = 3                // consecutive TCP failures before peer removal (D)
)
...
// rebuildOrder rebuilds peerOrder from joinOrder, keeping only live peers.
// Preserves join-arrival ordering — no sorting.
...
func (cs *ConsensusState) replicaVoteLoop() {
	for {
		...
		answer := strings.ToLower(strings.TrimSpace(cs.readLine()))
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: go run hotstuff.go network.go <node_id> [id=IP ...] [-m]")
		fmt.Fprintln(os.Stderr, "  node_id : 1–9  (port = 8000 + id)")
		fmt.Fprintln(os.Stderr, "  id=IP   : peer address, e.g.  2=192.168.1.2  (multi-laptop mode)")
		fmt.Fprintln(os.Stderr, "  -m      : Byzantine (malicious) node")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Same-machine (4 terminals):  go run hotstuff.go network.go 1")
		fmt.Fprintln(os.Stderr, "Multi-laptop :  go run hotstuff.go network.go 1 2=192.168.1.2 3=192.168.1.3 4=192.168.1.4")
		os.Exit(1)
	}
	...
	for _, a := range os.Args[2:] {
		...
		// Accept id=IP or id=IP:port (port is ignored — always 8000+id).
